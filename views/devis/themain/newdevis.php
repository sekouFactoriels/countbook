...
<div class="panel panel-default">
    <div class="panel-heading">
        <h4 class="panel-title"><i class="fa fa-shopping-cart"></i>&nbsp;&nbsp;Gestionnaire de Devis</h4>
    </div>

    <form class="form-horizontal" action="<?= Yii::$app->request->baseUrl . '/' . md5("devis_themain") ?>" id="ndevisForm" name="ndevisForm" method="post">
        <!-- DEBUT PANEL BODY -->
        <div class="panel-body">
            ...
            <div class="form-group">
                <label class="col-sm-4 control-label">Code de l'opération : </label>
                <div class="col-sm-4">
                            ...
                            <tbody id="divPannierDeVente">
                                ...
                                if (sizeof($dataPosted) > 0) {
                                    foreach ($dataPosted['selectectedELement'] as $key => $value) {
                                        ...
                                        echo '<tr id="row_' . $sub_selectectedELement->slimproductid . '" class="eachrow">
					                      <td>&#8226;<input autocomplete="off" type="hidden" name="selectectedELement[]" id="selectectedELement" value="' . $dataPosted['selectectedELement'][$key] . '"></td>
					                      <td>' . $sub_selectectedELement->productCode . ' : ' . $sub_selectectedELement->libelle . '</td>
					                      <td>' . Yii::$app->inventaireClass->getUdmLabel($sub_selectectedELement->udm) . '</td>
					                      <td>' . $sub_selectectedELement->qteDispo . '</td>
					                      <td><input autocomplete="off" class="form-control" onKeyup="calculSubTotal(this.id,' . $sub_selectectedELement->qteDispo . ',  ' . $sub_selectectedELement->prixUnitaireVente . ', ' . $sub_selectectedELement->slimproductid . ')" name="qteToBeSale[]" id="qteToBeSale_' . $sub_selectectedELement->slimproductid . '" value="' . $dataPosted['qteToBeSale'][$key] . '"></td>
					                      <td><input autocomplete="off" class="form-control totalmustpay" onKeypress="return false;" name="sousTotalLabel[]" id="sousTotalLabel_' . $sub_selectectedELement->slimproductid . '" value="' . $dataPosted['sousTotalLabel'][$key] . '"></td>
					                      <td><a id="' . $sub_selectectedELement->slimproductid . '" class="btn btn-circle btn-danger" onclick="deletThisRow(this.id)"><i Class="fa fa-times">&nbsp;</i></a></td>
					                    </tr>
					                  ';
                                    }
                                }
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <input autocomplete="off" class="form-control" id="totalMonetaire" name="totalMonetaire" value="<?= (sizeof($dataPosted) > 0) ? $dataPosted['totalMonetaire'] : Null ?>">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <input autocomplete="off" onKeyup="calculDetteVente()" class="form-control" id="montantPercu" name="montantPercu" value="<?= (sizeof($dataPosted) > 0) ? $dataPosted['montantPercu'] : Null ?>">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <input autocomplete="off" class="form-control" id="dettevente" name="dettevente" value="<?= (sizeof($dataPosted) > 0) ? $dataPosted['dettevente'] : Null ?>">
                </div>
            </div>
            ...
            <div class="panel-footer">
                <div class="row">
                    <div class="col-sm-9 col-sm-offset-3">
                        ...
                        <span>&nbsp;</span>
                        <a href="<?= Yii::$app->request->baseurl . '/' . md5('devis_themain') ?>" class="btn btn-circle btn-warning"><i class="glyphicon glyphicon-remove-circle"></i>&nbsp;Anuler</a>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="newProdCat">Enregistrement de Bond de commande</h4>
                        </div>
                        ...
                        <div class="modal-footer">
                            <a href="javascript:;" type="button" onclick="document.getElementById('action_key').value='<?= md5("enrgdevis") ?>'; document.getElementById('action_on_this').value='<?= md5("1") ?>'; document.getElementById('ndevisForm').submit();" class="btn btn-circle btn-success"><i class="glyphicon glyphicon-saved"></i>&nbsp;Oui</a>
...
<?php require_once('script/newdevis_js.php'); ?>