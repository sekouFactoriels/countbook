...
<!-- BEGIN FORM-->
<form action="" name="countbook_form" id="countbook_form" action="<?=yii::$app->request->baseurl.'/'.md5('client_themain') ?>" method="post">
<?=
  ...
  <div class="panel panel-default">
    ...
    <div class="panel-heading">
      <h4 class="panel-title"><i class="fa fa-users"></i>&nbsp;&nbsp;LISTE CLIENTS</h4>
    </div>
    <!-- LIBELLE TABLEAU -->
    ...
    <div class="panel-body">
      ...
      <table class="table">
        ...
        <tbody>
          ...
          {
            $i = 0;
            foreach($clients as $key => $each_client)
            {
              $compte_btn = '<a href="javascript:;" Class="btn btn-circle btn-primary" onclick="document.getElementById(\'action_key\').value=\''.md5(strtolower("charger_data_client")).'\';  document.getElementById(\'action_on_this\').value=\''.$each_client["id"].'\'; $(\'#countbook_form\').submit();">ACCEDER AU PROFIL</a>';

              ++$i;
              echo '<tr>
                <td>'.$i.'</td>
                <td>'.yii::$app->nonSqlClass->libeller_raison_sociale($each_client["statutcontact"]).'</td>
                <td>'.yii::$app->nonSqlClass->libeller_categorie_client($each_client["naturecontact"]).'</td>
                <td>'.$each_client["nom_appellation"].'</td>
                <td>'.$each_client["tel1"].'</td>
                <td>'.$compte_btn.'</td>
              </tr>';
            }
          }else{
...
<?php require_once('script/clients_js.php');?>
</form>