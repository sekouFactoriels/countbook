<form action="<?=yii::$app->request->baseurl.'/'.md5('client_themain') ?>" id="countbook_form" name="countbook_form" method="post">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <h4 class="panel-title"><i class="fa fa-list">&nbsp;</i>Liste des Recouvrement Payé</h4>
        </div>
                    ...
                    <tbody>
                        ...
                        if (is_array($bills) && sizeof($bills) > 0) {
                            foreach ($bills as $key => $each_bill) {
                                ...
                                $tt_paid = $each_bill["montantpercu"];
                                $dette = $each_bill["prixVenteAccorde"] - $tt_paid;
                                
                                if (isset($dette) && $dette > 0) {
                                    $badge = "danger";

                                    $action_btn = '<a href="javascript:;" Class="btn btn-circle btn-success" onclick="document.getElementById(\'action_key\').value=\''.md5(strtolower("preparer_rembourssement_client")).'\';  document.getElementById(\'action_on_this\').value=\''.$each_bill["codeVente"].'\'; document.getElementById(\'action_on_this_val\').value=\''.$each_bill["idbill"].'\'; $(\'#countbook_form\').attr(\'action\',\''.md5("paiement_themain").'\'); $(\'#countbook_form\').submit();">Ajouter paiement</a>';
                                ...
                                echo '
                                            <tr>
                                            <td>' . $key2 . '</td>
                                            <td>' . $each_bill["lastUpdate"] . '</td>
                                            <td style="font-size: 16px; font-weight: bold;">' . $each_bill["codeVente"] . '</td>
                                            <td style="font-size: 16px; font-weight: bold;">' . Yii::$app->venteClass->getClientname($each_bill["id_client"]) . '</td>
                                            <td>' . number_format($each_bill["prixTotalVente"]) . '</td>
                                            <td>' . number_format($each_bill["remiseMonetaire"]) . '</td>
                                            <td>' . number_format($each_bill["prixVenteAccorde"]) . '</td>
